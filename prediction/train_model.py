...
def main(training_params):
	...
	data_module = STRDataModule(
		...
		return_STR_len='_STR_len' in training_params['model_type']
	)
	...
	if training_params['model_type'] == 'InceptionPrePostModel':
	...
	elif training_params['model_type'] == 'InceptionPrePostModel_STR_len':
		net = prepost_models.InceptionPrePostModelSTRLen(
			...
			activation=training_params['activation'],
			dropout=training_params['dropout'],
			dense_dropout=training_params['dropout_dense'],
			dense_layers=training_params['dense_layer_sizes']
		)

	if '_STR_len' not in training_params['model_type']:
		...
		model = models.STRPrePostClassifierSTRLen(
			net,
			learning_rate=training_params['lr'],
			reduce_lr_on_plateau=training_params['reduce_lr_on_plateau'],
			reduce_lr_factor=training_params['reduce_lr_factor'],
