...
def score_fn(samp_adjs, agg_method='sum'):
	"""
...
def get_correlations(df, continuous_labels, agg_method,
	...
	scores = p_map(
		partial(score_fn, agg_method=agg_method),
		[df[df['sample_id'] == s_id] for s_id in df.sample_id.unique()],
		num_cpus=num_cpus,
		position=1,
		desc='Calculating feature scores',
		disable=True
	)
	str_lens = [
...
if __name__ == '__main__':
	...
	pattern_df = pattern_df.merge(
		samp_df[['sample_id', 'label', 'heterozygosity', 'entropy', 
					'minor_freq', 'motif', 'complement']],
		how='inner',
		on='sample_id'
	)
	pattern_df['alpha_motif'] = pattern_df.motif.apply(
	...
	for motif in pattern_df.alpha_motif.unique():
		motif_pattern_df = pattern_df[pattern_df.alpha_motif == motif]
		...
		for case, d, max_str_len, agg_method in tqdm(
			...
			# reduce df
			df = motif_pattern_df[
				(motif_pattern_df['case'] == case) 
				& (motif_pattern_df['d'] == d)
				& (motif_pattern_df['str_len'] <= max_str_len)
			]
			...
			corr_res_df['motif'] = motif

			all_corrs.append(corr_res_df)

	all_corrs_df = pd.concat(all_corrs, ignore_index=True)
	all_corrs_df.to_csv(
		os.path.join(label_version_dir, 'correlations_by_motif_cases{}_d{}_lens{}.csv'.format(
